...
query = [token for token in query if token not in stopwords or token == "!" or token == '«' or token == '»']
for i in range(len(query)):
    query[i] = lemmetizer.lemmatize(query[i])

# remove tokens that are not in inverted index to avoid key error
query = [token for token in query if token in sorted_inverted_index.keys() or token == "!" or token == '«' or token == '»'] 
...
if not_tokens:
    for i in range(len(not_tokens)):
        if not intersection:
            intersection = refrence - set(sorted_inverted_index[not_tokens[i]].keys())
            if not union:
                union = intersection
                continue 
        ...
        intersection = intersection.intersection(refrence - set(sorted_inverted_index[not_tokens[i]].keys()))
        union = union.union(refrence - set(sorted_inverted_index[not_tokens[i]].keys()))
...
if and_phrase:
    for phrase in and_phrase:
        ...
        for i in range(len(phrase)):
            if i == 0:
                phrase_intersection = set(sorted_inverted_index[phrase[i]].keys())
                continue
        ...
        for id in tmp_phrase_intersection:
            ...
            for j in range(1, len(phrase)):
                tmp = [x for x in tmp if x + j in sorted_inverted_index[phrase[j]][id]["positions"]]
...
if not_phrase:
    for phrase in not_phrase:
        ...
        intersection = intersection.intersection(refrence - phrase_intersection)
        union = union.union(refrence - phrase_intersection)

intersection = intersection - set(['total_frequency']) # remove total_frequency from union
